...
#과일별로 최대판매량에 연도를 추가해서 과일별, 연도별 최대 판매량 출력
aggregate(Sales~Fruit+Year, Fruits, max)

#apply(데이터, 행(1)/열(2), 함수) : matrix에서 유용하게 사용(행, 열을 대상으로 작업)
m1 = matrix(c(1,2,3,
              4,5,6), 2, by=T)
m1
...
lapply(c(l1, l2), max) #출력형태 : list형
sapply(c(l1, l2), max) #출력형태 : vector형
...
#데이터프레임이 아닌 벡터나 리스트 형태로 데이터가 있을 때 마치 데이터프레임처럼 연산을 해주는 함수
#단, 벡터들의 요소 개수가 동일해야 한다
...
#$ : dataframe$컬럼명은 입력할 데이터가 몇 개 안될 때 주로 사용
